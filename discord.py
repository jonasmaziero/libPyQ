...
def test_discord():
  ...
  from math import pi;  dtheta = (pi/2.0)/100.0;  theta = -dtheta
  for j in range(0,100):
  ...
  from distances import fidelity_pm;  from states import psi_1qb, rho_1qb
  FE1 = zeros(100);  vt = zeros(100); 
  vt[0] = 0.0;  psi = psi_1qb(0.0,0.0);  rho = rho_1qb(0.158, 0.16, 0.37);  FE1[0] = fidelity_pm(psi, rho);  print(vt[0],FE1[0])
  vt[1] = 0.2;  psi = psi_1qb(0.2,0.0);  rho = rho_1qb(0.202, 0.150, 0.332);  FE1[1] = fidelity_pm(psi, rho);  print(vt[1],FE1[1])
  vt[2] = 0.4;  psi = psi_1qb(0.4,0.0);  rho = rho_1qb(0.226, 0.172, 0.294);  FE1[2] = fidelity_pm(psi, rho);  print(vt[2],FE1[2])
  vt[3] = 0.6;  psi = psi_1qb(0.6,0.0);  rho = rho_1qb(0.226, 0.128, 0.266);  FE1[3] = fidelity_pm(psi, rho);  print(vt[3],FE1[3]) 
  vt[4] = 0.8;  psi = psi_1qb(0.8,0.0);  rho = rho_1qb(0.246, 0.144, 0.238);  FE1[4] = fidelity_pm(psi, rho);  print(vt[4],FE1[4])
  vt[5] = 1.0;  psi = psi_1qb(1.0,0.0);  rho = rho_1qb(0.212, 0.118, 0.178);  FE1[5] = fidelity_pm(psi, rho);  print(vt[5],FE1[5])
  vt[6] = 1.2;  psi = psi_1qb(1.2,0.0);  rho = rho_1qb(0.212, 0.118, 0.178);  FE1[6] = fidelity_pm(psi, rho);  print(vt[6],FE1[6])
  vt[7] = 1.4;  psi = psi_1qb(1.4,0.0);  rho = rho_1qb(0.212, 0.118, 0.178);  FE1[7] = fidelity_pm(psi, rho);  print(vt[7],FE1[7])
  vt[8] = pi/2.0;  psi = psi_1qb(pi/2.0,0.0);  rho = rho_1qb(0.212, 0.118, 0.178);  FE1[8] = fidelity_pm(psi, rho);  print(vt[8],FE1[8])
 
  import matplotlib.pyplot as plt
  plt.plot(vtheta,F1,label='F1(c1=0.6)');  plt.plot(vt,FE1,label='FE(c1=0.6)')
